...
for (i in 1:length(survival_status)) {
  ...
  cv_model <- glmnet::cv.glmnet(x_train, y_train, alpha = 1, family = "binomial", type.measure = "class", nfolds = (nrow(x_train)-1))
...
rownames(NormCounts.PCA) <- Age_df$Name
...
#Normnalise Data:
NormCounts.PCA[, 1:17] <- scale(NormCounts.PCA[, 1:17])
...
NormCounts.PCA <- cbind(NormCounts.PCA,umap_exprn)
...
NormCounts.PCA <- cbind(NormCounts.PCA, tsne)
...
ggplot(join, aes(x = "V18", y = "V19", color = Disease)) +
  geom_point(size = 3, alpha = 0.7) +
  scale_color_manual(values = c( "MDS" = "#FFA46B", "AML" = "#F1EAE4", "ALL" = "#5830F7")) +
  theme_minimal()

ggplot(join, aes(x = `V18`, y = `V19`, color = DonorAge)) +
  geom_point(size = 3, alpha = 0.7) +
  scale_color_gradient2(
    low = "#FFA46B",       # Low scores
    mid = "#F1EAE4",      # Midpoint (score = 0)
...
ggplot(join, aes(x = `V18`, y = `V19`, color = SurvivalFive)) +
  geom_point(size = 3, alpha = 0.7) +
  scale_color_manual(values = c( "Yes" = "green3", "No" = "orangered")) +
  theme_minimal()
